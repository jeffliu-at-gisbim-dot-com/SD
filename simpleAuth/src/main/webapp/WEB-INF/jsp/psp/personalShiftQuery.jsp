...
<!-- 
    程式說明
           依指定人員(employeeNo)月份(yearMonth), 查詢排班資料   
    
           注意:因為每一個Cell都是一筆資料, 很容易就會造成效能上的問題,
                       所以本程式不可以做大範圍的資料查詢, 需要限定查詢條件   
       
  @author jeff, juain farn, liu 劉君帆 (jeffliu@gisbim.com)    
  History:
     2017/03/07 first release
     2017/03/14 UI fix 修改主畫面, 顯示整個月的排班狀況, 讓使用者可以很快查到全月排班狀況
            ...
            <div class="easyui-panel" title="Search Filter" style="width:100%;max-width:100%;padding:30px 60px;">
                    <table>
                        <tr><td>Employee No:</td>
                            <td><input class="easyui-textbox" id="criteria_employeeNo" name="criteria_employeeNo" style="width:120px" value="${requestScope.employeeNo}">                           
                            </td>                            
                            <td> &nbsp; &nbsp;</td>
                            <td>Year Month:</td>
		...
		var jsApp = function() {
			...
			return {
				...
				ajax_findBy : function() {
					$('#dg').datagrid({
					    url: httpDispatcher +'/psp/personalShift/groupIndexList',
					    method:'post',
					    rownumbers:true,
					    pagination:true,
		                singleSelect:true,
